...
The benchmarks CLI is implemented in the `kaggle-cli` repository:
- **CLI argument parsing:** `src/kaggle/cli.py` — `parse_benchmarks()` function
- **API implementation:** `src/kaggle/api/kaggle_api_extended.py` — all `benchmarks_*_cli()` methods (starts at `# ---- Benchmarks CLI ----` section)
- **Documentation:** `docs/benchmarks.md` — full command reference
- **Tests:** `src/kaggle/test/test_benchmarks_cli.py` — unit tests with mocked API
- **Embedded templates:** `BENCHMARKS_SYNTAX_REF` and `BENCHMARKS_EXAMPLE_TASK` constants in `kaggle_api_extended.py`

## Key Implementation Details

### File Validation
- The CLI parses the Python file's AST to extract `@task` decorator names
- IPython magics (`%`, `!`, `%%`) are stripped before AST parsing (replaced with blank lines to preserve line numbers)
- The file is converted to `.ipynb` notebook format via `jupytext` using percent format (`py:percent`)

### Polling Mechanism
- Both `push --wait` and `run --wait` use a polling loop with configurable interval
- `--wait` with no value = `0` = wait indefinitely
- `--wait N` = timeout after N seconds
- Default poll interval is 10 seconds
- Terminal states for creation: `COMPLETED`, `ERRORED`
- Terminal states for runs: `COMPLETED`, `ERRORED`

### Download & Extraction
- Downloads are zip archives from the Kaggle server
- Downloads all runs in terminal states (both `COMPLETED` and `ERRORED`)
- Extracts to hierarchical directory: `{output}/{task}/{model_short_slug}/{run_id}/`
- Corrupt zip handling: keeps raw `.zip` file, prints warning, continues
- Idempotent: skips already-downloaded runs (checks if output directory exists)

### Error Messages
- Run errors are displayed in a separate colored `Errors:` section below the run table
- Each model's error is grouped with full traceback
- Task not found: friendly message with hint to use `kaggle b t list`
- Invalid models: friendly message with hint to use interactive selection
